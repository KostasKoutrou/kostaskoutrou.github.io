...
More details on the specific XPath queries is found in the sections below.


## MDAV Changes logging

An interesting and useful event related to all Microsoft Defender Antivirus (MDAV) features is the one with Event ID 5007 with description "Event when settings are changed". A lot of events with Event ID 5007 occur because it includes any change happening to the MDAV configuration, such as:
